...
/// The result of comparing two enum value domains.
...
pub(in crate::types) fn enum_comparison<'db>(
    db: &'db dyn Db,
    left: Type<'db>,
    right: Type<'db>,
    is_equality: bool,
    ...
    let truthiness =
        EnumDomainComparison::from_types(db, left, right, operator)?.truthiness(db, operator);
    Some(truthiness.map_or(EnumComparison::Unmodeled, EnumComparison::Known))
}
...
pub(in crate::types) fn enum_membership_constraint<'db>(
    ...
    let target = EnumValueSet::from_type(db, target)?;
    let members = EnumValueSet::from_type(db, members)?;
    if target.enum_class != members.enum_class {
        return None;
    }
    let comparison = SameEnumComparison::new(db, target, members, ComparisonOperator::Equality);
...
impl<'db> EnumDomainComparison<'db> {
    ...
    fn truthiness(&self, db: &'db dyn Db, operator: ComparisonOperator) -> Option<Truthiness> {
        match self {
            Self::SameClass(comparison) => comparison.truthiness(db, operator),
            Self::Projected(comparison) => Some(comparison.truthiness(operator)),
        }
    }
